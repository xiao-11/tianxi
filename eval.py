...
def evaluate(
    ...
    for idx_ in tqdm(range(data_size)):
    # for idx_ in range(data_size)
        sample = dataset[idx_]
        iid = sample['image_id']
        # img = Image.open(f"/data/lvbb1/rico/combined/{iid}.jpg")  # 返回 PngImageFile 对象
        img = f"/models/dataset/semantic_annotations/{iid}.png"
        ...
        fl_em_iou += em_iou
        fl_f1_iou += f1_iou
        fl_em_vh_index += em_vh_index
        fl_f1_vh_index += f1_vh_index
        # preds.append(response)
        # golds.append(sample['ground_truth'])
