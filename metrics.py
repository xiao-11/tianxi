...
### 新加
def sqa_numeric_metrics(prediction, ground_truths):
    """
    Computes metrics for numeric or nested list data (e.g., [-1] vs [[-1], [-1], [-1]]).
    
    Args:
        prediction: Model prediction (could be a single value or nested list).
        ground_truths: List of ground truth values (same structure as prediction).
    
    Returns:
        Tuple of (exact_match, f1_score).
    """
    # --- Step 1: Flatten prediction and ground_truths ---
    def flatten(data):
        flat = []
        if isinstance(data, list):
            for item in data:
                if isinstance(item, list):
                    flat.extend(flatten(item))  # 递归展平嵌套列表
                else:
                    flat.append(item)  # 添加基本类型（如数字）
        else:
            flat.append(data)  # 非列表直接添加
        return flat

    pred_flat = flatten(prediction)
    gt_flats = [flatten(gt) for gt in ground_truths]

    # --- Step 2: Exact Match (是否完全一致) ---
    exact_match = 0
    for gt_flat in gt_flats:
        if pred_flat == gt_flat:
            exact_match = 1
            break

    # --- Step 3: F1 Score (基于数值集合的 F1) ---
    def f1_for_lists(pred_list, gt_list):
        """计算两个数值列表的 F1 分数（类似文本 F1）"""
        common = collections.Counter(pred_list) & collections.Counter(gt_list)
    ...
    f1 = max([f1_for_lists(pred_flat, gt_flat) for gt_flat in gt_flats])

    return exact_match, f1


def sqa_s_metrics(prediction, ground_truths):
  # 纯文本答案匹配； EM 和 F1-Score
  ...
  if not ground_truths:
    return 0, 0
  prediction = normalize_squad(prediction)  # 移除标点、关系词，合并多余空格，转小写
